...
foreach ($eqLogics as $eqLogic)
{
  ...
  $HTML = '<div class="eqLogicDisplayCard cursor ' . $opacityV4 . '" data-eqLogic_id="' . $eqLogic->getId() . '" >';
  //$HTML .= "<center>";
  ...
  //$HTML .= "</center>";
  $HTML .= '<br><span class="name">' . $eqLogic->getHumanName(true, true) . '</span>';
  $HTML .= '<span class="hiddenAsCard displayTableRight hidden">';
  $HTML .= '<span class="label">' . $JExtname  .  '</span>';
  $HTML .= ($eqLogic->getIsVisible() == 1) ? '<i class="fas fa-eye" title="{{Équipement visible}}"></i>' : '<i class="fas fa-eye-slash" title="{{Équipement non visible}}"></i>';;
  $HTML .= '</span>';
  $HTML .= '</div>';
...
<div class="<?php echo $ShowSideBar;?> eqLogicThumbnailDisplay">
  ...
  <div class="eqLogicThumbnailContainer">
    <div class="cursor eqLogicAction" style="color:#00979C" data-action="add" >
      <i class="fas fa-plus-circle" ></i><br>
      <span>{{Ajouter}}</span>
    </div>
    <div class="cursor eqLogicAction" style="color:#00979C" data-action="gotoPluginConf" >
      <i class="fas fa-wrench" ></i><br>
      <span>{{Configuration}}</span>
    </div>
    <div class="cursor eqLogicAction" style="color:#00979C" data-action="bt_healthSpecific" >
      <i class="fas fa-medkit" ></i><br>
      <span>{{Santé}}</span>
    </div>
    <div class="cursor eqLogicAction" style="color:#00979C" data-action="bt_docSpecific" >
    ...
    if (config::byKey('ActiveExt', 'jeedouino', false)) {
      ?>
      <div class="cursor eqLogicAction" style="color:#00979C" data-action="bt_jeedouinoExt" >
        <i class="fas fa-screwdriver" ></i><br>
        <span>{{JeedouinoExt}}</span>
      </div>
      <?php
    }
    ?>
  ...
  <div class="input-group" style="margin-bottom:5px;">
    ...
    <div class="input-group-btn">
      ...
      </a><a class="btn roundedRight hidden" id="bt_pluginDisplayAsTable" data-coreSupport="1" data-state="1"><i class="fas fa-grip-lines"></i></a>
    </div>
  </div>
  ...
  <div class="<?php echo $ShowSideBar;?> eqLogic eqLogic_active" data-eqLogic_id = "" style="display: none;">
    ...
    <ul class="nav nav-tabs" role="tablist">
      ...
      <li role="presentation"class="control pintab"><a href="#pinstab" aria-controls="profile" role="tab"data-toggle="tab"><i class="fas fa-wrench"></i> {{Pins / GPIO}}</a></li>
      <li role="presentation"class="sketchstab"><a href="#sketchstab" aria-controls="profile" role="tab"data-toggle="tab"  ><i class="fas fa-code"></i> {{Sketchs}}</a></li>
    </ul>
        ...
        <form class="form-horizontal">
          <fieldset>
                    ...
                    if (config::byKey('ActiveExt', 'jeedouino', false))
                    {
                      ...
                      if ($ListExtIP != '')
                      {
                        foreach ($ListExtIP as $ip)
                        {
                          ...
                          if (is_array($UsbMap) and !empty($UsbMap))
                          {
                          ...
                          else echo '<option value="null" disabled>{{Aucune info remontée.}}</option>';
            ...
            <div class="esp8266">
              ...
              <div class="form-group">
                <label class="col-sm-3 control-label">{{Mot de passe WIFI}}</label>
                <div class="col-sm-3 input-group">
                  <input type="text" class="inputPassword eqLogicAttr configuration form-control" data-l1key="configuration" data-l2key="wifi_pass" placeholder="Mot de passe"/>
      ...
      <div role="tabpanel" class="tab-pane" id="sketchstab">
        ...
        foreach ($eqLogics as $eqLogic)
        {
          ...
          if (substr($ModeleArduino,0,1) == 'a')
          {
            ...
            if (file_exists($SketchFileNameUSB))
            {
