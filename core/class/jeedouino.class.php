...
class jeedouino extends eqLogic {
	...
	public static function cron() {
		...
		if ($CronStep == $BootTime) jeedouino::StartAllDaemons();
		$CronStep++;
	...
	public static function cron5() {
		...
		foreach ($eqLogics as $eqLogic) {
			...
			if (config::byKey($board_id . '_HasDemon', 'jeedouino', 0) and config::byKey('Auto_' . $board_id, 'jeedouino', 0)) {
				...
				if (!jeedouino::StatusBoardDaemon($board_id, 0, $ModeleArduino)) jeedouino::StartBoardDaemon($board_id, 0, $ModeleArduino);
			}
		}
	}
	...
	public static function deamon_start($_debug = false) {
		...
		foreach ($eqLogics as $eqLogic) {
			...
			if (config::byKey($board_id . '_HasDemon', 'jeedouino', 0)) {
				...
				self::StartBoardDaemon($board_id, 0, $board);
				sleep(2);
			}
		}
	...
	public static function deamon_stop() {
		...
		foreach ($eqLogics as $eqLogic) {
			...
			if (config::byKey($board_id . '_HasDemon', 'jeedouino', 0)) {
				jeedouino::log('debug', __('-=-= ' . $board_id . ' =-=-', __FILE__));
				list(, $board, $usb) = self::GetPinsByBoard($board_id);
				self::StopBoardDaemon($board_id, 0, $board);
				sleep(2);
			}
		}
	...
	public static function execSH($exec2, $log) {
		if (count(system::ps('dpkg')) > 0 || count(system::ps('apt')) > 0) {
			throw new \Exception(__('Installation impossible car il y en a déjà une en cours. Veuillez patienter.', __FILE__));
		}
		$exec  = "echo '======= Start of apt -y --fix-broken install ======='\n";
		$exec .= system::getCmdSudo() . " apt -y --fix-broken install\n";
		$exec .= "echo '======= Start of dpkg --configure -a --force-confdef ======='\n";
		$exec .= system::getCmdSudo() . " dpkg --configure -a --force-confdef\n";
		$exec .= "echo '======= Start of apt autoremove ======='\n";
		$exec .= system::getCmdSudo() . " apt -y autoremove\n";
		if (file_exists('/tmp/jeedouinoSH')) shell_exec(system::getCmdSudo() . ' rm /tmp/jeedouinoSH');
		file_put_contents('/tmp/jeedouinoSH', $exec . $exec2);
		shell_exec(system::getCmdSudo() . ' chmod +x /tmp/jeedouinoSH');
		exec(system::getCmdSudo() . '/bin/bash /tmp/jeedouinoSH >> ' . $log . ' 2>&1 &');
	}
	/*************************** Méthodes d'instance **************************/

	public static function StartAllDaemons($EqIDarr = '') {
		$EqLogics = [];
		if (!is_array($EqIDarr)) {
			$EqLogics = eqLogic::byType('jeedouino');
	...
	public static function GetPinsByBoard($arduino_id)	// Renvoie la liste des pins suivant le type de carte
	{
	...
	public static function GetPinMode($arduino_id) {
	...
	public function ConfigurePinMode($ForceStart = true) 	// Paramétrage du mode des pins)
	{
		...
		if (($ModeleArduino != '') and (($PortArduino == 'rj45arduino') or ($PortArduino == 'usbarduino'))) {
			...
			$PinMode = self::GetPinMode($arduino_id);
				...
				switch ($board) {
					...
					case 'gpio':
						if ($ForceStart or $ForceStart == '1') {
							self::StartBoardDaemon($arduino_id, 0, $ModeleArduino);   // on démarre le démon si nécéssaire.
							sleep(2);
						}
				}
				switch ($ModeleArduino) {
					...
					case 'piGPIO40':
						...
						$reponse = self::SendToBoardDaemon($arduino_id, $BootMode, $ModeleArduino);
						if ($reponse != 'BMOK') {
							jeedouino::log('debug', ucfirst($ModeleArduino) . __(' - ERREUR ENVOI CONFIGURATION BootMode (eqID: ', __FILE__) . $arduino_id . ' ) - Réponse: ' . $reponse);
							config::save($arduino_id . '_StatusDemon', false, 'jeedouino');
							break;
						}
					case 'piface':
					case 'piPlus':
						...
						$reponse = self::SendToBoardDaemon($arduino_id, $PinMode, $ModeleArduino);
						if ($reponse != 'COK') {
							jeedouino::log('debug', ucfirst($ModeleArduino) . __(' - ERREUR ENVOI CONFIGURATION PinMode (eqID: ', __FILE__) . $arduino_id . ' ) - Réponse: ' . $reponse);
							config::save($arduino_id . '_StatusDemon', false, 'jeedouino');
						}
						break;
					default:
						...
						$PinMode = 'C' . $PinMode . 'C';
						config::save($arduino_id . '_PinMode', $PinMode, 'jeedouino');
						$reponse = self::SendToArduino($arduino_id, $PinMode, 'PinMode', 'COK');
						if ($reponse != 'NOK') $reponse = self::SendToArduino($arduino_id, 'B' . config::byKey($arduino_id . '_choix_boot', 'jeedouino', '2') . 'M', 'BootMode', 'BMOK');
						if ($PortArduino == 'rj45arduino') {
							if ($reponse != 'NOK') $reponse = self::SendToArduino($arduino_id, 'E' . $arduino_id . 'Q', 'BoardEQ', 'EOK');
							if ($reponse != 'NOK') $reponse = self::SendToArduino($arduino_id, 'I' . self::GetJeedomIP() . 'P', 'BoardIP', 'IPOK');
						}
	...
	public static function SendToArduino($arduino_id, $message, $keyword, $waitfor) {
		$my_arduino = eqLogic::byid($arduino_id);
		...
		if ($reponse != $waitfor) {
			...
			else jeedouino::log('debug', __('ERREUR ENVOI CONFIGURATION ', __FILE__) . $keyword . __(' ( équipement ', __FILE__) . $arduino_id . ' - ' . $name . ' ) - Réponse :' . $reponse);
		}
		return $reponse;
	}
	...
	public static function ConfigurePinValue($pins_id, $value, $arduino_id) {
		...
		if ($ModeleArduino == 'piface' or $ModeleArduino == 'piGPIO26' or $ModeleArduino == 'piGPIO40' or $ModeleArduino == 'piPlus') {
			...
			if ($PinValue != '')  $reponse = self::SendToBoardDaemon($arduino_id, $PinValue, $ModeleArduino);
		} else // Arduinos
		{
		...
		if ($reponse != 'SOK' and $reponse != 'SMOK') {
			...
			if ($reponse != 'SOK' and $reponse != 'SMOK') {
				jeedouino::log('debug', __('Erreur d\'envoi de la commande (EqID : ', __FILE__) . $arduino_id . ' ) - Réponse reçue :' . $reponse);
				return false;
			}
		}
		return true;
	...
	public static function SendToBoard($arduino_id, $message) {
		$my_arduino = eqLogic::byid($arduino_id);
		$PortArduino = $my_arduino->getConfiguration('datasource');
		$IPArduino = $my_arduino->getConfiguration('iparduino');
		$ipPort = $my_arduino->getConfiguration('ipPort');
		...
		if ($PortArduino == 'rj45arduino')		// envoi sur reseau local
		{
			...
			jeedouino::log('debug', __('Réponse envoyée par l\'arduino : ', __FILE__) . $reponse);
			return $reponse;
		} else			// envoi sur usb
		{
			return self::SendToBoardDaemon($arduino_id, 'USB=' . $message, 'USB');
		}
		return 'NOK';
	}
	...
	public static function GiveMeFreePort($TypePort = '') {
		$ipPortArr = array();
		$PortDemonArr = array();
		$ipPort = 8000;
		$PortDemon = 8080;
	...
	public static function CreateJeedouinoControl() {
		...
		foreach ($cmds as $cmd) {
			...
			// si équipement effacé ou desactivé ou si arduino usb mis vers lan avec même id
			if (!is_object($eqLogic) or $eqLogic->getIsEnable() == 0 or (trim($eqLogic->getConfiguration('datasource')) == 'rj45arduino' and substr($eqLogic->getConfiguration('arduino_board'), 0, 1) == 'a')) $cmd->remove();
	...
	public static function updateControlCmd($eqLogic_id, $StatusDemon) {
	...
	public static function updateDemons() {
		$eqLogics = eqLogic::byType('jeedouino');
		...
		foreach ($eqLogics as $eqLogic) {
			...
			if (($eqLogic->getConfiguration('datasource') == 'usbarduino' and substr($ModeleArduino, 0, 1) == 'a') or $ModeleArduino == 'piface' or $ModeleArduino == 'piGPIO26' or $ModeleArduino == 'piGPIO40' or $ModeleArduino == 'piPlus') {
				...
				$StatusDemon = jeedouino::StatusBoardDaemon($eqLogic->getId(), 0, $ModeleArduino);
	...
	public static function AddIPJeedouinoExt($ip) {
		// On a reçu une nouvelle ip ? Si oui on l'ajoute à la liste
		/** @var array */
		$ListExtIP = config::byKey('ListExtIP', 'jeedouino', [$ip]);
		if (!in_array($ip, $ListExtIP)) {
			$ListExtIP[] = $ip;
			config::save('ListExtIP', $ListExtIP, 'jeedouino');
		}
	}
	public static function AddIDJeedouinoExt($ip) {
	...
	public static function IPfromIDJeedouinoExt($id) {
	...
	public static function SaveIPJeedouinoExt($jeedouino_ext) {
		$ip = $jeedouino_ext['IP'];
	...
	public static function GetJeedouinoExt($ip) {
		$id = trim(config::byKey('ID-' . $ip, 'jeedouino', ''));
	...
	public static function CleanIPJeedouinoExt() {
		/** @var array */
	...
	public static function allJeedouinoExt() {
	...
	public static function RemoveJeedouinoExt($ip) {
	...
	public static function SendJeedouinoExt($jeedouino_ext, $Noinstall = false) {
		jeedouino::log('debug', __('Envoi des fichiers JeedouinoExt sur ', __FILE__) . $jeedouino_ext['IP']);
		...
		if ($Noinstall) {
			...
			foreach (eqLogic::byType('jeedouino') as $eqLogic) {
				if ($eqLogic->getConfiguration('iparduino') == $jeedouino_ext['IP']) {
					...
					jeedouino::ReStartBoardDaemon($eqLogic->getId(), 0, $eqLogic->getConfiguration('arduino_board'));
				}
			}
		}
		return true;
	...
	public static function SshCmdJeedouinoExt($connection, $preCmd, $cmd) {
	...
	public static function SshDelJeedouinoExt($jeedouino_ext, $local, $distant) {
	...
	public static function SshGetJeedouinoExt($jeedouino_ext, $local, $distant) {
		jeedouino::log('debug', __('Téléchargement via SSH d\'un fichier de JeedouinoExt depuis l\'IP: ', __FILE__) . $jeedouino_ext['IP']);
		if (!$connection = ssh2_connect($jeedouino_ext['IP'], $jeedouino_ext['sshPort'])) {
			jeedouino::log('error', __('Connection SSH impossible sur ', __FILE__) . $jeedouino_ext['IP']);
			return false;
		} else {
	...
	public static function getExtLog($_log) {
		$log = file($_log);
		if ($log !== false) return $log;
		else return [];
	}
	public static function SendSSHCmdsJeedouinoExt($jeedouino_ext, $cmds) {
		jeedouino::log('debug', __('Envoi de commandes SSH pour JeedouinoExt sur ', __FILE__) . $jeedouino_ext['IP']);
		if (!$connection = ssh2_connect($jeedouino_ext['IP'], $jeedouino_ext['sshPort'])) {
			jeedouino::log('error', __('Connection SSH impossible sur ', __FILE__) . $jeedouino_ext['IP']);
			return false;
		} else {
	...
	public static function FilterDaemon($DemonType) {
	...
	public static function SendToBoardDaemon($board_id, $message, $DemonType)	// Envoi de message au Démon
	{
		$DemonTypeF = self::FilterDaemon($DemonType);
	...
	public static function StartBoardDaemon($board_id, $useless = 0, $DemonType)	// Démarre le Démon
	{
		$DemonTypeF = self::FilterDaemon($DemonType);
		...
		if ($jeedomIP == $IPBoard or $IPBoard == '127.0.0.1' or config::byKey($board_id . '_IpLocale', 'jeedouino', 1))
			...
			$result = self::CallSlaveExt('StartBoardDemonCMD', array(
				'plugin' => 'jeedouino',
				'eqLogic' 		=> $board_id,
				'DemonType' 	=> $DemonType,
				'ipPort' 		=> $ipPort,
		...
		if ($PinMode != 'none') {
			if (($DemonTypeF == 'PiGpio') or ($DemonTypeF == 'PiPlus')) {
				...
				$reponse = self::SendToBoardDaemon($board_id, $BootMode, $DemonType);
				if ($reponse != 'BMOK') {
			...
			while ($_try < 2 and $reponse != 'COK') {
				...
				$reponse = self::SendToBoardDaemon($board_id, $PinMode, $DemonType);
	...
	public static function StartBoardDemonCMD($board_id = '', $DemonType, $ipPort, $jeedomIP, $JeedomPort, $JeedomCPL, $PiPlusBoardID, $PiFaceBoardID, $PortDemon, $_portUSB)	// Démarre le Démon
	{
		$DemonTypeF = self::FilterDaemon($DemonType);
		...
		// Generation du fichier du demon + fallback
		$DemonFileName = $jeedouinoPATH . $jeedouinoFile . '.py';

		$MasterFile = file_get_contents($DemonFileName);
		if ($MasterFile) {
			if (($DemonTypeF == 'PiGpio') or ($DemonTypeF == 'PiPlus')) {
				// boot mode
				$BootMode = 'BootMode = ' . config::byKey($board_id . '_' . $DemonTypeF . '_boot', 'jeedouino', '0');
				$MasterFile = str_replace('BootMode = False', $BootMode, $MasterFile);
	...
	public static function StopBoardDaemon($board_id, $useless = 0, $DemonType)	// Stoppe le Démon
	{
		$DemonTypeF = self::FilterDaemon($DemonType);
		if ($DemonTypeF == null) return;
		// Arrét soft
		...
		$reponse = self::SendToBoardDaemon($board_id, 'EXIT=1', $DemonType);
	...
	public static function ForceStopBoardDemon($board_id, $useless = 0, $DemonType)	// Stoppe le(s) processus du Démon local ou déporté
	{
		$DemonTypeF = self::FilterDaemon($DemonType);
		if ($DemonTypeF == null) return;
		...
		if ($jeedomIP == $IPBoard) self::StopBoardDemonCMD($board_id, $DemonType);   // sur local  (master ou esclave)
			...
			self::CallSlaveExt('StopBoardDemonCMD', array(
				'plugin' => 'jeedouino',
				'eqLogic' => $board_id,
				'DemonType' => $DemonType
			));
	...
	public static function StopBoardDemonCMD($board_id = '', $DemonType)	// Stoppe le(s) processus du Démon local
	{
		$DemonTypeF = self::FilterDaemon($DemonType);
		...
		foreach ($processus as $process) {
			jeedouino::log('debug', __('Tentative de suppression (kill) du process : ', __FILE__) . $process);
	...
	public static function StatusBoardDaemon($_board_id = 0, $forceCache = 0, $DemonType)	 // Démon en marche ???
	{
		$DemonTypeF = self::FilterDaemon($DemonType);
		if ($DemonTypeF == null) return false;
		if ($_board_id == 0) return false;
		...
		$reponse = self::SendToBoardDaemon($_board_id, 'PING=1', $DemonType); // On le PINGue
		...
		if ($reponse != 'PINGOK') {
			...
			$reponse = self::SendToBoardDaemon($_board_id, 'PING=2', $DemonType); // On rePINGue
			if ($reponse == 'PINGOK') {
	...
	public function ChangePortDemon($board_id = 0) {
		if ($board_id == 0) $board_id = $this->getid();
	...
	public static function ReStartBoardDaemon($board_id, $useless = 0, $DemonType)	// Redémarre le Démon
	{
		$DemonTypeF = self::FilterDaemon($DemonType);
		if ($DemonTypeF == null) return;

		config::save('StartDemons', 1, 'jeedouino');
		if (self::StatusBoardDaemon($board_id, 1, $DemonType)) {
			self::StopBoardDaemon($board_id, 0, $DemonType);
		}
		usleep(1500000); // 1.5 secondes
		self::StartBoardDaemon($board_id, 0, $DemonType);
		config::save('StartDemons', 0, 'jeedouino');
	}
	public static function EraseBoardDemonFile($board_id, $useless = 0, $DemonType)	// Cherche le Démon pour effacer son fichier jeedouinoUSB_*.py
	{
		$DemonTypeF = self::FilterDaemon($DemonType);
		if ($DemonTypeF == null) return;
		...
		if ($jeedomIP == $IPBoard) self::EraseBoardDemonFileCMD($board_id, $DemonType);   // sur local ArduinoUsb (master ou esclave)
			...
			self::CallSlaveExt('EraseBoardDemonFileCMD', array(
				'plugin' => 'jeedouino',
				'eqLogic' => $board_id,
				'DemonType' => $DemonType
			));
	...
	public static function EraseBoardDemonFileCMD($board_id = '', $DemonType)	// Efface le fichier python généré pour le Démon
	{
		$DemonTypeF = self::FilterDaemon($DemonType);
		if ($DemonTypeF == null or $board_id == '') return;
		...
		if ($BoardEQ !== false and $BoardEQ !== null)	// Si création équipement, il n'y a pas de eqLogic pour comparer
		{
			...
			if ($ModeleArduino != '') {
				...
				if (substr($ModeleArduino, 0, 3) == 'esp') {
					...
					if (strpos($wifi_ssid, ' ') !== false) jeedouino::log('error', 'EqID ' . $arduino_id . __(' - Votre SSID WIFI contient un/des espaces. Cela peut poser des problèmes de connection.', __FILE__));
					if (strpos($wifi_pass, ' ') !== false) jeedouino::log('error', 'EqID ' . $arduino_id . __(' - Votre mot de passe WIFI contient un/des espaces. Cela peut poser des problèmes de connection.', __FILE__));
	...
	public static function SendPRM($eqLogic) {
		$arduino_id = $eqLogic->getId();
		list(, $board, $usb) = jeedouino::GetPinsByBoard($arduino_id);
		...
		if ($reponse != 'OK') jeedouino::log('error', __('Problème d\'envoi de la commande SetJeedomCFG sur JeedouinoExt déporté (eqID: ', __FILE__) . $arduino_id . ' ) - ' . $reponse);
		...
		if ($ToSend) {
			$Fallback = [];
			$Fallback['BootMode'] = config::byKey($arduino_id . '_' . $DemonName . '_boot', 'jeedouino', '0');
		...
		if ($this->getIsEnable() == 0) {
			...
			if ($IPBoard != '' and filter_var($IPBoard, FILTER_VALIDATE_IP) !== false and $AlreadyKilled == false) {
				...
				switch ($board) {
					case 'arduino':
						if ($usb) {
							self::StopBoardDaemon($arduino_id, 0, $board);
						}
						break;
					case 'piface':
					case 'piplus':
		...
		if (($ModeleArduino != '') and (($PortArduino == 'rj45arduino') or ($PortArduino == 'usbarduino'))) {
			...
			foreach ($Arduino_pins as $pins_id => $pin_datas) {
				...
				if ($myPin != 'not_used') {
					...
					$cmd_list[$LogicalId . 'a'] = array(
						'name' 			=> $ID_pins . '_' . $myPinN,
						'type' 			=> $myType,
						'subtype' 		=> $mySubType,
						'tempo' 		=> $tempo,
						'value' 		=> $value,
					...
					if ($double_cmd != '') {
						...
						$list_order_nb++;
						$cmd_list[$LogicalId . 'b'] = array(
							'name' 			=> $ID_pins . '_' . $double_cmdN,
							'type' 			=> $myType,
							'subtype' 		=> $mySubType,
							'tempo' 		=> $tempo,
							'value' 		=> $value2,
					...
					if ($triple_cmd != '') {
						$list_order_nb++;
						$cmd_list[$LogicalId . 'c'] = array(
							'name' 			=> $ID_pins . '_' . $triple_cmd,
							'type' 			=> $myType,
							'subtype' 		=> $mySubType,
							'tempo' 		=> $tempo,
							'value' 		=> $value3,
					...
					if ($quadruple_cmd != '') {
						$list_order_nb++;
						$cmd_list[$LogicalId . 'd'] = array(
							'name' 			=> $ID_pins . '_' . $quadruple_cmd,
							'type' 			=> $myType,
							'subtype' 		=> $mySubType,
							'tempo' 		=> $tempo,
							'value' 		=> $value4,
					...
					if (($myType == 'action') and ($mySubType == 'other' or $mySubType == 'slider') and ($myPin != 'trigger') and ($myPin != 'servo') and ($myPin != 'WS2811') and ($myPin != 'compteur_pulldown') and ($myPin != 'compteur_pullup')) {
						...
						$cmd_list[$LogicalId . 'i'] = array(
							'name' 			=> __('Etat_Pin_', __FILE__) . $ID_pins,
							'type' 			=> 'info',
							'subtype' 		=> $mySubType,
							'tempo' 		=> '0',
							'value' 		=> '0',
			...
			if ($this->getConfiguration('ActiveCmdAll')) {
				$cmd_list['ALLON'] = array(
					'name' 			=> __('ALL_LOW', __FILE__),
					'type' 			=> 'action',
					'subtype' 		=> 'other',
					'tempo' 		=> '0',
					'value' 		=> '0',
				...
				$list_order_nb++;
				$cmd_list['ALLOFF'] = array(
					'name' 			=> __('ALL_HIGH', __FILE__),
					'type' 			=> 'action',
					'subtype' 		=> 'other',
					'tempo' 		=> '0',
					'value' 		=> '1',
				...
				$list_order_nb++;
				$cmd_list['ALLSWT'] = array(
					'name' 			=> __('ALL_SWITCH', __FILE__),
					'type' 			=> 'action',
					'subtype' 		=> 'other',
					'tempo' 		=> '0',
					'value' 		=> '1',
				...
				$list_order_nb++;
				$cmd_list['ALLPLSELOW'] = array(
					'name' 			=> __('ALL_PULSE_LOW', __FILE__),
					'type' 			=> 'action',
					'subtype' 		=> 'other',
					'tempo' 		=> '00007',
					'value' 		=> '0',
				...
				$list_order_nb++;
				$cmd_list['ALLPLSEHIGH'] = array(
					'name' 			=> __('ALL_PULSE_HIGH', __FILE__),
					'type' 			=> 'action',
					'subtype' 		=> 'other',
					'tempo' 		=> '00007',
					'value' 		=> '1',
	...
	public static function ProbeDelay($arduino_id, $ProbeDelay = 5) {
		...
		switch ($board) {
			...
			case 'piplus':
				...
				$reponse = jeedouino::SendToBoardDaemon($arduino_id, $message, $board);
	...
	public static function GetJeedomPort() // on récupère  le port de l'adresse du maître si dispo
	{
		if (config::byKey('ActiveJLAN', 'jeedouino', false)) {
...
class jeedouinoCmd extends cmd {
	public function execute($_options = null) {
		...
		if ($this->getLogicalId() == 'refresh') {
			...
			jeedouino::StartBoardDaemon($this->getConfiguration('boardid'), 0, $this->getConfiguration('demontype'));
			//sleep(3);
			jeedouino::updateDemons();
			return;
		} elseif (strpos($this->getLogicalId(), 'StopDaemon') !== false) {
			//jeedouino::log( 'debug', 'CMD StartDaemon $this->getConfiguration(demontype) = ' . $this->getConfiguration('demontype'));
			jeedouino::StopBoardDaemon($this->getConfiguration('boardid'), 0, $this->getConfiguration('demontype'));
			//sleep(3);
			jeedouino::updateDemons();
			return;
		}
		...
		if ($this->getType() == 'action') {
			try {
				if ($this->getSubType() == 'other') {
					...
					if ($_options === null) return false;
					//jeedouino::log( 'debug','_ - $_options = '. json_encode($_options));
