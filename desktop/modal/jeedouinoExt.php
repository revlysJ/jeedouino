...
<script>
	...
	if ( confirm('Etês-vous sûr de vouloir vider ce fichier distant ?' ) )
	{
		...
		$('#div_jeedouinoExtAlert').showAlert({message: '<i class="fa fa-spinner fa-spin fa-fw"></i> Purge du fichier de log ' + logf + ' en cours.', level: 'success'});
		$.ajax({
			type: "POST",
			...
			success: function (data) {
				...
				setTimeout(function(){ $('#div_jeedouinoExtAlert').hide(); }, 3000);
			}
		});
	};
});
	...
	$('.jeedouinoExtAction[data-action=save]').on('click',function(){
		...
		$.ajax({
			...
			success: function (data) {
                ...
                setTimeout(function() {
                    $('#div_jeedouinoExtAlert').showAlert({message: '{{Sauvegarde réussie}} ', level: 'success'});
   ...
   $('.bt_StartDemon').on('click', function () {
       var jextid = $(this).attr('data-jextid');
        $.ajax({// fonction permettant de faire de l'ajax
            ...
            $('#div_jeedouinoExtAlert').showAlert({message: '{{Démarrage du démon demandé.}}', level: 'success'});
        }
    });
    });
   ...
   $('.bt_restartDemon').on('click', function () {
        $.ajax({// fonction permettant de faire de l'ajax
            ...
            $('#div_jeedouinoExtAlert').showAlert({message: '{{Re-démarrage du démon demandé.}}', level: 'success'});
        }
    });
    });
   ...
   $('.bt_stopDemon').on('click', function () {
        $.ajax({// fonction permettant de faire de l'ajax
            ...
            //$('#ul_plugin .li_plugin[data-plugin_id=jeedouino]').click();   // recharge la page config du plugin
            $('#div_jeedouinoExtAlert').showAlert({message: '{{Arret du démon demandé.}}', level: 'success'});
        }
    });
    });
    ...
    $('.bt_AutoReStart').on('click', function () {
        ...
        $.ajax({// fonction permettant de faire de l'ajax
            ...
            success: function (data) { // si l'appel a bien fonctionné
                ...
                $('#div_jeedouinoExtAlert').showAlert({message: '{{AutoRestart du démon modifié.}}', level: 'success'});
