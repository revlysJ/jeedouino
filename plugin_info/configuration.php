...
<ul class="nav nav-tabs" role="tablist" id="tab_jeedouino">
  <li role="presentation" class="active"><a href="#tab_logs" role="tab" data-toggle="tab"><i class="fas fa-pencil-square-o"></i> {{Options}}</a></li>
  <li role="presentation"><a href="#tab_dep" role="tab" data-toggle="tab"><i class="fas fa-certificate"></i> {{Dépendances}}</a></li>
  <li role="presentation"><a href="#tab_demon" role="tab" data-toggle="tab"><i class="fas fa-university"></i> {{Démons}}</a></li>
  <li role="presentation"><a href="#tab_sketch" role="tab" data-toggle="tab"><i class="fas fa-download"></i> {{Sketchs}}</a></li>
  <li role="presentation"><a href="#tab_docker" role="tab" data-toggle="tab"><i class="fas fa-rss"></i> {{Conf. Docker}}</a></li>
</ul>
<div class="tab-content" id="tab2_jeedouino" style="height: unset !important;">
  <div role="tabpanel" class="tab-pane active" id="tab_logs">
    <br/>
...
<div role="tabpanel" class="tab-pane" id="tab_dep">
  <br/>
  <?php
  $dep = '';
...
<div role="tabpanel" class="tab-pane" id="tab_demon">
  <br/>
  <?php $html = '
  ...
  foreach ($eqLogics as $eqLogic)
  {
    ...
    if (!$a_lan)
    {
      ...
      if ($Sketch != '')
      {
        ...
        {
          $html .= '<div class="col-lg-5"><a href="plugins/jeedouino/data/JeedouinoUSB_' . $board_id . '.ino" class="btn btn-info" download target="_blank" ><i class="fas fa-download"></i>{{ SketchUSB ( EqID : ' . $board_id . ' ) }}</a></div>';
          ...
          <div class="col-lg-5">
          <a href="core/php/downloadFile.php?pathfile=plugins/jeedouino/data/JeedouinoUSB_' . $board_id . '.ino" class="btn btn-info" download target="_blank" ><i class="fas fa-download"></i>{{ Télécharger le Sketch a mettre dans l\'arduino ( EqID : ' . $board_id . ' ) }}</a>
          </div></div>';
        }
        else
        {
          $html .= '<div class="col-lg-5"><a href="core/php/downloadFile.php?pathfile=plugins/jeedouino/data/JeedouinoUSB.ino" class="btn btn-info"  title="{{ Télécharger le Sketch à mettre dans l\'arduino }}" download target="_blank"><i class="fas fa-download"></i> SketchUSB</a></div>';
        }
      }
      $html .= '</td></tr>';
    }
    else
    {
      ...
      if ($esp)
      {
        ...
        <div class="col-lg-5">
        <a href="core/php/downloadFile.php?pathfile=plugins/jeedouino/data/JeedouinoESP_' . $board_id . '.ino" class="btn btn-info" download target="_blank" ><i class="fas fa-download"></i>{{ Télécharger le Sketch a mettre dans l\'esp ( EqID : ' . $board_id . ' ) }}</a>
        </div></div>';
      }
      else
      {
        ...
        <div class="col-lg-5">
        <a href="core/php/downloadFile.php?pathfile=plugins/jeedouino/data/JeedouinoLAN_' . $board_id . '.ino" class="btn btn-info" download target="_blank" ><i class="fas fa-download"></i>{{ Télécharger le Sketch a mettre dans l\'arduino ( EqID : ' . $board_id . ' ) }}</a>
        </div></div>';
      }
    }
  ...
  <div role="tabpanel" class="tab-pane" id="tab_sketch">
    <br/>
    <form class="form-horizontal">
      <fieldset>
        <?php
  ...
  <div role="tabpanel" class="tab-pane" id="tab_docker">
    <br/>
...
<script>
  document.getElementById('tab2_jeedouino').addEventListener('click', function(event) {
    var _target = null
    if (_target = event.target.closest('.bt_installUpdate')) {
      domUtils.ajax({
        type: "POST",
        ...
        success: function (data) {
          ...
          jeedomUtils.showAlert({message: '{{Le système est en cours de mise à jour.}}', level: 'success'});
        }
      })
      return
    }
    if (_target = event.target.closest('.bt_installSerial')) {
      domUtils.ajax({
        type: "POST",
        url: "plugins/jeedouino/core/ajax/jeedouino.ajax.php",
        data: {
        ...
        success: function (data) {
          ...
          //$('#ul_plugin .li_plugin[data-plugin_id=jeedouino]').click();   // recharge la page config du plugin
          jeedomUtils.showAlert({message: '{{Le module Serial pour Python est en cours d\'installation.}}', level: 'success'});
        }
      })
      return
    }
    if (_target = event.target.closest('.bt_installPiPlus')) {
      domUtils.ajax({
        type: "POST",
        url: "plugins/jeedouino/core/ajax/jeedouino.ajax.php",
        data: {
        ...
        success: function (data) {
          ...
          //$('#ul_plugin .li_plugin[data-plugin_id=jeedouino]').click();   // recharge la page config du plugin
          jeedomUtils.showAlert({message: '{{Les dépendances IO.PiPlus sont en cours d\'installation.}}', level: 'success'});
        }
      })
      return
    }
    if (_target = event.target.closest('.bt_installDS18B20')) {
      domUtils.ajax({
        type: "POST",
        url: "plugins/jeedouino/core/ajax/jeedouino.ajax.php",
        data: {
        ...
        success: function (data) {
          ...
          //$('#ul_plugin .li_plugin[data-plugin_id=jeedouino]').click();   // recharge la page config du plugin
          jeedomUtils.showAlert({message: '{{L\'installation de la dépendance danjperron/BitBangingDS18B20 est en cours.}}', level: 'success'});
        }
      })
      return
    }
    if (_target = event.target.closest('.bt_installGPIO')) {
      domUtils.ajax({
        type: "POST",
        url: "plugins/jeedouino/core/ajax/jeedouino.ajax.php",
        data: {
        ...
        success: function (data) {
          ...
          //$('#ul_plugin .li_plugin[data-plugin_id=jeedouino]').click();   // recharge la page config du plugin
          jeedomUtils.showAlert({message: '{{Les dépendances RPI.GPIO sont en cours d\'installation.}}', level: 'success'});
        }
      })
      return
    }
    if (_target = event.target.closest('.bt_installPIFACE')) {
      domUtils.ajax({
        type: "POST",
        url: "plugins/jeedouino/core/ajax/jeedouino.ajax.php",
        data: {
        ...
        success: function (data) {
          ...
          //$('#ul_plugin .li_plugin[data-plugin_id=jeedouino]').click();   // recharge la page config du plugin
          jeedomUtils.showAlert({message: '{{Les dépendances PIFACEDIGITALIO sont en cours d\'installation.}}', level: 'success'});
        }
      })
      return
    }
    if (_target = event.target.closest('.bt_StartDemon')) {
      domUtils.ajax({
        ...
        success: function (data) {
          ...
          //$('#ul_plugin .li_plugin[data-plugin_id=jeedouino]').click();   // recharge la page config du plugin
          jeedomUtils.showAlert({message: '{{Le démon va démarrer.}}', level: 'success'});
        }
      })
      return
    }
    if (_target = event.target.closest('.bt_restartDemon')) {
      domUtils.ajax({
        ...
        success: function (data) {
          ...
          //$('#ul_plugin .li_plugin[data-plugin_id=jeedouino]').click();   // recharge la page config du plugin
          jeedomUtils.showAlert({message: '{{Le démon va (re)démarrer.}}', level: 'success'});
        }
      })
      return
    }
    if (_target = event.target.closest('.bt_stopDemon')) {
      domUtils.ajax({
        ...
        success: function (data) {
          ...
          //$('#ul_plugin .li_plugin[data-plugin_id=jeedouino]').click();   // recharge la page config du plugin
          jeedomUtils.showAlert({message: '{{Le démon va s\'arreter.}}', level: 'success'});
        }
      })
      return
    }
