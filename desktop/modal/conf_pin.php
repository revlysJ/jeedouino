...
if (isset($_GET['id']))
{
  ...
  if ($non_defini)
  {
    ...
    ?>
    <div id='div_alertpins' class="alert alert-success" style="display: none;">
      <center><h4>{{Sauvegarde effectuée}}</h4>
        <br><br>
        {{Paramétrages réussis. }}
        <br><br>
        {{/!\ Pensez à sauver l'équipement ensuite pour envoyer la config à la carte et (re)générer les commandes.}}
      </center>
    </div>
        ...
        if (substr($ModeleArduino, 0, 2) != 'pi')
          {
            ...
            echo '<option value="3" selected style="color: #3c763d!important;">{{Sauvegarde sur JEEDOM - Toutes les pins sorties mises suivant leur sauvegarde dans Jeedom. Lent, Jeedom requis sinon pins mises à HIGH.}}</option>';
          ...
          elseif (substr($ModeleArduino,0,6) == 'piGPIO')
          {
            ...
            echo '<option  selected="selected" value="0">{{Toutes les pins sorties mises à LOW au démarrage du démon}}</option>';
          ...
          switch ($ModeleArduino)
          {
            ...
            if (method_exists('virtual', 'copyFromEqLogic'))
            {
              ...
              $Virtuels = '<option selected="selected" >{{Aucun}}</option>' . $Orphelins . $Virtuels . '</select>';
            }
            ...
            foreach ($Arduino_pins as $pins_id => $pin_datas)
            {
              ...
              if ($pin_datas['disable'] == '1')
              {
                ...
                if ($sda == 'SDA')
                {
                  ...
                  $TmpPins .= '<option  selected="selected" value="not_used">{{Non utilisée}}</option>';
              ...
              if ($pins_id < $UserPinsBase)
              {
                ...
                else if ($ModeleArduino == 'esp01' or $ModeleArduino == 'esp07' or $ModeleArduino == 'espMCU01' or $ModeleArduino == 'esp32dev')
                {
                  ...
                  else
                  {
                    ...
                    $OtherPins[] = '<option selected="selected" value="not_used">{{Non utilisée}}</option>';
          ...
          <script>
          document.getElementById('conf_Pin').addEventListener('click', function(event) {
            var _target = null
            if (_target = event.target.closest('.bt_CptDelay')) {
              if (document.querySelector('.configKeyPins[data-l1key="<?php echo $arduino_id; ?>_CptDelay"]') == null) return;
              domUtils.ajax({
                type: "POST",
                ...
                data: {
                  ...
                  CptDelay : document.querySelector('.configKeyPins[data-l1key="<?php echo $arduino_id; ?>_CptDelay"]').jeeValue()
                },
                dataType: 'json',
                ...
                success: function (data) {
                  ...
                  jeedomUtils.showAlert({message: '{{La valeur délai de RéArm Event des compteurs a bien été envoyée.}}', level: 'success'});
                }
              })
              return
            }
            if (_target = event.target.closest('.bt_bounceDelay')) {
              if (document.querySelector('.configKeyPins[data-l1key="<?php echo $arduino_id; ?>_bounceDelay"]') == null) return;
              domUtils.ajax({
                type: "POST",
                url: "plugins/jeedouino/core/ajax/jeedouino.ajax.php",
                data: {
                  action: "bounceDelay",
                  boardid : <?php echo $arduino_id; ?>,
                  bounceDelay : document.querySelector('.configKeyPins[data-l1key="<?php echo $arduino_id; ?>_bounceDelay"]').jeeValue()
                },
                dataType: 'json',
                error: function (request, status, error) {
                  domUtils.handleAjaxError(request, status, error);
                ...
                success: function (data) {
                  ...
                  jeedomUtils.showAlert({message: '{{La valeur délai anti-rebonds des compteurs a bien été envoyée.}}', level: 'success'});
                }
              })
              return
            }
            if (_target = event.target.closest('.bt_ProbeDelay')) {
              if (document.querySelector('.configKeyPins[data-l1key="<?php echo $arduino_id; ?>_ProbeDelay"]') == null) return;
              domUtils.ajax({
                type: "POST",
                url: "plugins/jeedouino/core/ajax/jeedouino.ajax.php",
                data: {
                  action: "ProbeDelay",
                  boardid : <?php echo $arduino_id; ?>,
                  ProbeDelay : document.querySelector('.configKeyPins[data-l1key="<?php echo $arduino_id; ?>_ProbeDelay"]').jeeValue()
                },
                dataType: 'json',
                error: function (request, status, error) {
                  domUtils.handleAjaxError(request, status, error);
                ...
                success: function (data) {
                  ...
                  jeedomUtils.showAlert({message: '{{La valeur délai de relève des sondes a bien été envoyée.}}', level: 'success'});
                }
              })
              return
            }
            if (_target = event.target.closest('.bt_savebackup_pins')) {
              //$.hideAlert();
              jeedom.config.save({
                // configuration: $('#backup_pins').getValues('.configKeyPins')[0],
                configuration: document.getElementById('backup_pins').getJeeValues('.configKeyPins')[0],
                ...
                success: function () {
                  jeedom.config.load({
                    ...
                    success: function (data) {
                      ...
                      document.getElementById('backup_pins').setJeeValues(data, '.configKeyPins')
                      modifyWithoutSave = false;
                      // $('#md_modal').dialog('close');

                      document.querySelector('#div_alertpins').seen();
                      document.querySelector('#backup_pins').unseen();
              ...
              domUtils.ajax({
                ...
                data: {
                  action: 'addKey',
                  value: json_encode(document.getElementById('backup_pins').getJeeValues('.configKeyPins')[0]),
                  plugin: 'jeedouino',
                },
                dataType: 'json',
                error: function (request, status, error) {
                ...
                success: function (data) {
                  ...
                  modifyWithoutSave = false;
                  // setTimeout(function() {
                  //   event.target.closest('div.jeeDialog').unseen()
                  // }, 500)
                  //event.target.closest('div.jeeDialog').unseen()
                }
              });
            ...
            // if (_target = event.target.closest('#backup_pins .cmdAction[data-action=remove]')) {
            //   _target.closest('tr').unseen();
            //   return
            // }
          })
          // if (document.getElementById('bt_adduser_pins')) document.getElementById('bt_adduser_pins').addEventListener('click', function(event) {
          //   var tr = document.querySelector('#user_cmd tbody tr:first');
          // })

          //initTableSorter();
          </script>
          <?php
// a voir si variable existe au depart
            if (config::byKey($arduino_id . '_choix_boot', 'jeedouino', '7') != '7')
            echo "<script>
                  jeedom.config.load({
                    configuration: document.getElementById('backup_pins').getJeeValues('.configKeyPins')[0],
                    plugin: 'jeedouino',
