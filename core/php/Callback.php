...
if (isset($_GET['BoardEQ'])) {
	...
	if ($eqLogic !== false and $eqLogic !== null) {
		...
		if (isset($_GET['NODEP'])) {
			if ($eqLogic->getIsEnable() == 0) jeedouino::StopBoardDaemon($arduino_id, 0, $ModeleArduino);
		...
		if (isset($_GET['PINGME'])) {
			...
			$result = jeedouino::StatusBoardDaemon($arduino_id, 1, $ModeleArduino); // Force le cache
		}
		if (isset($_GET['THREADSDEAD'])) {
			...
			jeedouino::ReStartBoardDaemon($arduino_id, 0, $ModeleArduino);
		}
			...
			if ($PinMode != '') // and config::byKey($arduino_id . '-ForceStart', 'jeedouino', '0') == '0')
			{
				switch ($ModeleArduino) {
					...
					case 'piPlus':
						...
						$DemonTypeF = jeedouino::FilterDaemon($ModeleArduino);
						if ($DemonTypeF == 'USB') $PinMode = 'USB=' . $PinMode;
						$reponse = jeedouino::SendToBoardDaemon($arduino_id, $PinMode, $DemonTypeF);
